...
{
	internal static class MaintainCompositeForeignKey2Keys
	{
		...
		private static void DeleteProcedure(SqlConnection sqlConnection)
		{
			...
			sqlStringBuilder.AppendLine("			objects.name = 'MaintainCompositeForeignKey2Keys'");
			sqlStringBuilder.AppendLine("	)");
			sqlStringBuilder.AppendLine(")");
		...
		private static void CreateProcedure(SqlConnection sqlConnection)
		{
			...
			sqlStringBuilder.AppendLine("	MaintainCompositeForeignKey2Keys");
			...
			sqlStringBuilder.AppendLine("			AND");
			sqlStringBuilder.AppendLine("			columns.name IN (@foreignKey1Name,@foreignKey2Name)");
			sqlStringBuilder.AppendLine("			AND");
			sqlStringBuilder.AppendLine("			childColumns.name IN (@primaryKey1Name,@primaryKey2Name)");
			sqlStringBuilder.AppendLine("		) foreignKeys ) != 2");
			sqlStringBuilder.AppendLine("	BEGIN");
			sqlStringBuilder.AppendLine("");
			...
			sqlStringBuilder.AppendLine("		BEGIN");
			sqlStringBuilder.AppendLine("			EXEC SP_EXECUTESQL @sql, N'@tablename NVARCHAR(128), @foreignKey1Name NVARCHAR(128), @foreignKey2Name NVARCHAR(128), @primaryTablename NVARCHAR(128), @primaryKey1Name NVARCHAR(128), @primaryKey2Name NVARCHAR(128)', @tablename = @tablename, @foreignKey1Name = @foreignKey1Name, @foreignKey2Name = @foreignKey2Name, @primaryTablename = @primaryTablename, @primaryKey1Name = @primaryKey1Name, @primaryKey2Name = @primaryKey2Name");
			sqlStringBuilder.AppendLine("		END");
			sqlStringBuilder.AppendLine("	END");
			sqlStringBuilder.AppendLine("	ELSE");
			sqlStringBuilder.AppendLine("	BEGIN");
