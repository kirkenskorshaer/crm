...
namespace SystemInterface.Dynamics.Crm
{
	public class Materiale : AbstractCrm
	{
		...
		public static Materiale ReadCalculationNeed(DynamicsCrmConnection dynamicsCrmConnection, Func<string, string> getResourcePath)
		{
			...
			Materiale materiale = StaticCrm.ReadFromFetchXml(dynamicsCrmConnection, path, (connection, entity) => new Materiale(connection, entity)).FirstOrDefault();
		...
		public void RemoveStaleMaterialeBehov(Func<string, string> getResourcePath)
		{
			...
			List<MaterialeBehov> materialeBehovList = StaticCrm.ReadFromFetchXml(Connection, xDocument, (connection, entity) => new MaterialeBehov(connection, entity));
		...
		{
			BehovDefinitioner = MaterialeBehovDefinition.FindMaterialeBehovDefinitionPerMateriale(Connection, Id, getResourcePath);
		}
		...
		public void AddMissingMateriale(MaterialeProcessState state, Func<string, string> getResourcePath)
		{
			...
			pakker.OrderByDescending(pakke => pakke.new_stoerrelse);

			List<Account> accounts = new List<Account>();
			GetAccountsToAddMaterialeBehov(BehovDefinitioner, accounts, state, getResourcePath);
			...
			foreach (Account account in accounts)
			{
				...
				if (state.withMaterialeBehov)
				{
					currentSize = account.CountMaterialeBehov(Id, getResourcePath);
				}

				currentSize = FindMaterialeNeed(pakker, materialePakkerOnAccount, materialeNeededCount, currentSize);

				CreateMaterialeBehov(account, materialePakkerOnAccount);
			}

			state.LargestAccountId = accounts.Select(account => account.Id).OrderByDescending(id => id).FirstOrDefault();
			state.AccountsProcessed = accounts.Count;
		}

		private static int FindMaterialeNeed(List<MaterialePakke> pakker, Dictionary<MaterialePakke, int> materialePakkerOnAccount, decimal materialeNeededCount, int currentSize)
		{
		...
		private void CreateMaterialeBehov(Account account, Dictionary<MaterialePakke, int> materialePakkerOnAccount)
		{
			foreach (KeyValuePair<MaterialePakke, int> materialePakkeAndCount in materialePakkerOnAccount)
			{
				MaterialeBehov behov = new MaterialeBehov(Connection)
				{
					...
					modtagerid = GetModtager(account),
		...
		private Guid? GetModtager(Account account)
		{
		...
		private List<Account> GetAccountsToAddMaterialeBehov(List<MaterialeBehovDefinition> behovDefinitioner, List<Account> accounts, MaterialeProcessState state, Func<string, string> getResourcePath)
		{
